...
import { getEntryDetail, publishEntryRevision } from '@/api/entry'
...
async function loadEntryForEdit() {
  ...
  try {
    const [entryDetail, worldDetail] = await Promise.all([
      getEntryDetail(worldId.value, entryId.value),
      getWorldDetail(worldId.value)
    ])

    entry.value = entryDetail
    world.value = worldDetail
    form.title = entryDetail.title
    form.content = entryDetail.content
    tags.value = [...entryDetail.tags]
    tagInput.value = ''
  } catch (error) {
    entry.value = null
    world.value = null
    errorMessage.value = getErrorMessage(error, '词条档案暂时无法加载，请稍后重试。')
  } finally {
    loading.value = false
  }
}
...
async function handleSubmit() {
  ...
  try {
    const updatedEntry = await publishEntryRevision(worldId.value, entryId.value, {
    ...
    await router.push({
      name: 'entry-detail',
      params: {
        worldId: updatedEntry.worldId,
        entryId: updatedEntry.entryId
      }
    })
  } catch (error) {
    errorMessage.value = getErrorMessage(error, '修订暂时无法发布，请稍后重试。')
  } finally {
...
<template>
  <main class="edit-entry-page">
    <section v-if="loading" class="entry-edit-shell page-container">
      <div class="entry-state">
        <p>正在调取词条修订档案...</p>
      </div>
    </section>

    <section v-else-if="!entry || !world || !canEditWorld" class="entry-edit-shell page-container">
      <div class="entry-state entry-state--error">
        <h1>{{ entry && world ? '你当前无法编辑这个词条' : '词条档案暂时不可用' }}</h1>
        <p>{{ errorMessage || '请返回词条详情页确认权限后再试。' }}</p>
        <div class="state-actions">
          <RouterLink
            class="state-button state-button--primary"
            :to="{ name: 'entry-detail', params: { worldId, entryId } }"
          >
            返回词条详情
          </RouterLink>
          <button type="button" class="state-button" @click="loadEntryForEdit">重新加载</button>
        </div>
      </div>
    </section>

    <section v-else class="entry-edit-shell page-container">
      <nav class="entry-breadcrumb" aria-label="页面路径">
        ...
        <span>/</span>
        <RouterLink :to="{ name: 'entry-detail', params: { worldId, entryId } }">
          {{ entry.title }}
        </RouterLink>
        <span>/</span>
        <strong>编辑词条</strong>
      ...
      <header class="entry-edit-header">
        <div>
          <p class="eyebrow">Revision Desk</p>
          <h1>发布词条修订</h1>
        </div>
        <p>修改标题、正文和标签后发布为新版本；历史版本仍留在词条档案里供回溯。</p>
      </header>
      ...
      <div class="entry-edit-layout">
        ...
        <aside class="entry-preview">
          ...
          <section class="world-panel">
            ...
            <span>当前版本 {{ entry.currentRevisionId }}</span>
